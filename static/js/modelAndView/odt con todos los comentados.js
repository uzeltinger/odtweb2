...
OdtFullView = Backbone.View.extend({
  ...
  events: {
	...
	'click input[type=button]#rechazar'     : 'MailRechazo',
  ...
  definirODT: function(){
   ...
   MailRechazo: function() {
       if(confirm("Está seguro que quiere rechazar la ODT Nro " + this.model.get("codigoODT") + "?")) {
          ...
          win.close();
    var CorreoRechazo =  "window.location.href = 'http://hwnt04/odtweb/include/rechazo.asp?codigo='  + this.model.get('codigoODT')" 
     
	 ////  window.location.href = 'http://hwnt04/odtweb/include/rechazo.asp?codigo='  + this.model.get('codigoODT') 
	
	//*************************************************************************************************
     //Guille.. se me ocurrió lo siguiente.. dado que un window.open de un link SI funciona el cerrado automático de la página, lo que hice fué crear el temporal.asp que hace un request.querystring del mailto_link 
   //y desde ahí se podría generar el correo en asp.. una vez hecho esto, como tiene timeout, la ventana asp se va a cerrar, dejandote el mail abierto.. no sé si es viable y si va a funcionar pero estoy probando. Por lo pronto
   //la ventana que genera, la cierra sin problemas luego del timeout.. 
   //o sea esto window.open(mailto_link, 'emailWindow') es lo que no funciona.. pero si reemplazamos el mailto_link por  http://hwnt04/odtweb/temporal.asp? y le pasamos el mailto_link en el url, después si te deja
   //cerrar la ventana con setTimeout(function() { win.close() }, 500); 
   
   // Lo ultimo que hay que hacer ahora y no me está saliendo es: desde temporal.asp es obtener el contenido de la variable mailto_link que pasa como string desde js al asp y convertirlo en una version legible para asp
   //que te permita generar el mail desde esa página temporal.asp
  //*************************************************************************************************
  
	//mailto_link = 'mailto:' + this.model.get('MNcontacto') + '?subject=' + 'ODT | Rechazo ODT Nro: ' + this.model.get("codigoODT") + '&body=' + 'Defina aquí el motivo del rechazo';
    //window.location = mailto:' + this.model.get('MNcontacto') + '?subject=' + 'ODT | Rechazo ODT Nro: ' + this.model.get("codigoODT") + '&body=' + 'Defina aquí el motivo del rechazo';
  //alert("Gracias.");

  //cadena = this.model.get('MNcontacto')
  
  
  //win = window.open("http://hwnt04/odtweb/temporal.asp?MNcontacto=cadena"
   //setTimeout(function() {win.close()}, 5000);
  //    var win = window.open(mailto_link, 'emailWindow');
//	setTimeout(function() { win.close() }, 500);
	
	//win.document.write("<p>Puede cerrar esta ventana ahora</p>");
	//win.focus();
	//if (win && win.open) win.close();
	// estudiar aquí   window.location.replace('http://hwnt04/odtweb/include/rechazo.asp?codigo=' + this.model.get('MNcontacto') + '")';

  //win = window.open("hwnt04/include/rechazo.asp?codigo="  + this.model.get('codigoODT') + ",MNcontacto" + this.model.get('MNcontacto')
